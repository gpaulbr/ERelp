...
def textosTreinamento(diretorioTrabalho, categUtilizadas, dicionarios, pasta):
    ...
    for arquivo in arquivos:
        texto = open(pasta + arquivo).read()
        if(not arquivo.endswith('~')):#evita arquivos temporários
            textos, relacoesTotalAux = moduloExtrai.processaTags(diretorioTrabalho, categUtilizadas, texto, dicionariosFormat)
...
def formataDic(dicionarios):
    ...
    #lista com os dicionarios formatados
    dicionariosFormat = list()
    ...
    for dic in dicionarios:
        ...
        #monta o dicionario usando o primeiro item de cada linha
        for i in range(len(dicSplited)):
