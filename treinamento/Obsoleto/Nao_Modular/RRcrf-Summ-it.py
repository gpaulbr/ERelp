		...
		i=0
		    ...
		    if '(local)' in word.lower():
		        saida4.write(word2+':\n')
                        for a_string in all_strings:
                            if word.lower() in a_string.lower():
                                a_string = a_string.replace(string, "").replace("(ORG)", "").replace("(LOCAL)", "").replace("(PES)", "")
                                a_string = a_string.replace(string, "")
                                a_string = string_replaces(a_string)
                                saida4.write(a_string + '\n')
		    saida4.write("\n")
...
def processaTags(trainingTweets, dicionariosFormat): #retorna lista no formato
    ...
    frases = [frase.replace('\r\n','\n') for frase in frases]#tira quebra de linha do windows
    frases = [frase.replace('\r','\n') for frase in frases]#tira quebra de linha do mac
    frases = [frase.replace('\n\n\n','\n') for frase in frases]#tira quebra de linha do mac
    frases = [frase.replace('\n\n','\n') for frase in frases]#tira quebra de linha do mac


    frases = [frase.replace('$.','$. O') for frase in frases]#tira quebra de linha do windows
    frases = [frase.replace('$,','$, O') for frase in frases]#tira quebra de linha do mac
    frases = [frase.replace('$:','$: O') for frase in frases]#tira quebra de linha do mac
    frases = [frase.replace('$;','$; O') for frase in frases]#tira quebra de linha do windows
    frases = [frase.replace('$(','$( O') for frase in frases]#tira quebra de linha do mac
    frases = [frase.replace('$)','$) O') for frase in frases]#tira quebra de linha do mac
    ...
    #frases = [re.sub(r'\$[^S]+\n', '', frase) for frase in frases]#retira as $pontuação
    ...
    for f in frases:
	    ...
	    salvadic = 0
            ...
            if(t != '' and len(t)>2):#verifica se a linha não está vazia
                print t
...
def RR(labelled, test, iteracao):
    ...
    descritorEntradaCerto = open(diretorioTrabalho+'/DescritorEntrada.txt','a')
    ...
    for texto in test:
		...
		descritorEntradaCerto.write(texto[0] + '\n')
	    elif(linha == 1):
            	descritor.write('\n' + texto[0] + '\n')
		descritorEntradaCerto.write('\n' + texto[0] + '\n')
...
############################ Numero de Acertos ################################
				...
				listadescritor.write(' '+w[0]['palavraOriginal']+" ")
				...
				listachaves.write(sInicio[0]['clas'] + '-'+sFim[0]['clas']+ ' PARCIAL '+ '{'+texto[0]+'}' +'\n')				
				listadescritor.write(sInicio[0]['clas'] + '-'+sFim[0]['clas']+ ' PARCIAL '+ '{'+texto[0]+'}'  +'\n')
				...
				listadescritor.write(sInicio[0]['clas'] +'-'+sFim[0]['clas'] +' TOTAL '+ '{'+texto[0]+'}' +'\n')
				listachaves.write(sInicio[0]['clas'] +'-'+sFim[0]['clas'] +' TOTAL '+ '{'+texto[0]+'}' +'\n')
				if(sInicio[0]['clas'] == 'ORG' and sFim[0]['clas'] == 'ORG'):
					orgorg += 1
				if(sInicio[0]['clas'] == 'ORG' and sFim[0]['clas'] == 'LOCAL'):
					orglocal += 1
