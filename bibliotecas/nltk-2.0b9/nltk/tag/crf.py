...
###########################################################################################################
        (fd, test_file) = mkstemp('.txt', 'test'+str(threading.current_thread()))
	print str(threading.current_thread()) + "TESTE"
###########################################################################################################
...
##############################################################################################
        (fd, train_file) = mkstemp('.txt', 'train'+str(threading.current_thread()))
##############################################################################################
    ...
    def _build_crf_info(corpus, gaussian_variance, default_label,
        ...
        if weight_groups is None:
            weight_groups = [WeightGroup(name=l, src='.*',
                                                 dst=re.escape(l))
                             for l in labels]
        ...
        for src in labels:
            ...
            if add_end_state:
                if src == '__end__':
                    ...
                    final_cost = '+inf'
            state_info = State(src, initial_cost, final_cost, [])
            for dst in labels:
                state_weight_groups = [wg.name for wg in weight_groups
                                       if wg.match(src, dst)]
                state_info.transitions.append(
                    Transition(dst, dst, state_weight_groups))
...
class CRFInfo(object):
    ...
    @staticmethod
    def _read(etree):
        states = [State._read(et) for et in
                  etree.findall('states/state')]
        weight_groups = [WeightGroup._read(et) for et in
...
class State(object):
        ...
        @staticmethod
        def _read(etree):
            transitions = [Transition._read(et)
                           for et in etree.findall('transitions/transition')]
            return State(etree.get('name'),
...
class Transition(object):
        ...
        @staticmethod
        def _read(etree):
            return Transition(etree.get('destination'),
                                      etree.get('label'),
...
class WeightGroup(object):
        ...
        @staticmethod
        def _read(etree):
            return WeightGroup(etree.get('name'),
                                       etree.get('src'),
                                       etree.get('dst'),
                                       etree.get('features'))
